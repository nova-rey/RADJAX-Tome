"""Pure selected-payload tail diagnostics shared by producers and packages."""
...
def long_tail_diagnostics(
    *,
    ...
    long_tail_class = classify_long_tail(
        effective_top_k=effective_top_k, vocab_size=vocab_size, policy=policy
    )
    saturated = effective_top_k >= min(dynamic_top_k_max, vocab_size)
...
def classify_long_tail(
    *, effective_top_k: int, vocab_size: int, policy: LongTailPolicy
) -> str:
...
def semantic_tail_tag(
    *, long_tail_class: str, top_token_texts: Iterable[str] = ()
) -> str:
    pieces = [
...
def long_tail_summary(items: Iterable[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "mean_effective_top_k": float(sum(effective)) / float(len(effective))
        if effective
        else 0.0,
    ...
    *,
    ...
    if saturated:
        warnings.append(
            ...
            f"effective_top_k={effective_top_k}, dynamic_top_k_max={dynamic_top_k_max}"
        )
    return warnings
