"""Materialize explicit native-v3 Student-consumption v4 sidecar evidence.

V4 is an opt-in successor to the published v2/v3 profiles. It reuses v2's
...
SIDECAR_DIRECTORY = "student_consumption/v4"
ASSIGNMENT_PATH = _V2_ASSIGNMENT_PATH.replace("/v2/", "/v4/")
OBSERVED_STATISTICS_PATH = _V2_OBSERVED_STATISTICS_PATH.replace("/v2/", "/v4/")
TARGET_ROWS_PATH = _V2_TARGET_ROWS_PATH.replace("/v2/", "/v4/")
EXAMPLE_REGISTRY_PATH = _V2_EXAMPLE_REGISTRY_PATH.replace("/v2/", "/v4/")
ROW_RANGES_PATH = _V2_ROW_RANGES_PATH.replace("/v2/", "/v4/")
DELIVERY_RECEIPT_PATH = _V2_DELIVERY_RECEIPT_PATH.replace("/v2/", "/v4/")
AUTHORITY_REFERENCE_PATH = _V2_AUTHORITY_REFERENCE_PATH.replace("/v2/", "/v4/")
SELECTED_PASSPORT_INDEX_PATH = _V2_SELECTED_PASSPORT_INDEX_PATH.replace("/v2/", "/v4/")
...
class NativeV3StudentConsumptionV4Materialization:
    """V4 paths and counts emitted into a package staging directory."""
...
def materialize_native_v3_student_consumption_v4(
    artifact_root: Path,
    *,
    destination_root: Path | None = None,
    selection_integration_config_hash: str | None = None,
) -> NativeV3StudentConsumptionV4Materialization:
    """Write the closed v4 evidence sidecar from native production facts."""
    ...
    _write_json(v4_dir / "delivery_receipt.json", _delivery_receipt(source_root))
    _write_json(
        v4_dir / "authority_reference.json",
        _authority_reference(
            source_root,
            selection_integration_config_hash=selection_integration_config_hash,
        ),
    )
    return NativeV3StudentConsumptionV4Materialization(
        root=root,
        role_paths={
            key: value.replace("/v2/", "/v4/") for key, value in v2.role_paths.items()
        },
        example_count=v2.example_count,
        assignment_count=v2.assignment_count,
    )
...
def _delivery_receipt(source_root: Path) -> dict[str, Any]:
    ...
    if delivery_path not in {"one_pass_full", "two_pass_rerun_selected"}:
        raise ValueError("native v4 artifact has no recognized delivery_path evidence")
    return {
...
def _authority_reference(
    ...
    if selection_integration_config_hash is not None:
        ...
        if existing not in {None, selection_integration_config_hash}:
            raise ValueError("native v4 selection authority disagrees with identity")
        authority["selection_integration_config_hash"] = (
            selection_integration_config_hash
        )
    if "selection_integration_config_hash" not in authority:
        raise ValueError("native v4 artifact has no selection integration authority")
    if (
        not {"score_pass_authority_hash", "score_pass_authority_hash_v1"}
        & authority.keys()
    ):
        raise ValueError("native v4 artifact has no score-pass authority")
    return {
...
def _first_object(root: Path, *relatives: str) -> dict[str, Any]:
    ...
    raise ValueError("native v4 artifact has no delivery evidence")
