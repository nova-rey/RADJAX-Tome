...
def _ref(path: str, raw: bytes, schema: str, *, record_count: int | None = None) -> dict[str, Any]:
    result: dict[str, Any] = {
        "path": path,
        "sha256": _digest(raw),
        "size_bytes": len(raw),
...
def _closed_record(record: Mapping[str, Any], payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    merged.setdefault("effective_top_k_fraction_of_vocab", float(merged["effective_top_k"]) / float(merged["vocab_size"]))
...
def _authority_and_policy(config: Any, context: Mapping[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    authorities = dict(context.get("authorities") or {})
    provenance = json.loads(Path(config.teacher_model_provenance_path).read_text(encoding="utf-8"))
    ...
    tokenizer = required_digest(provenance.get("tokenizer_hash"), "tokenizer_vocabulary")
    score = required_digest(authorities.get("score_pass_authority_hash"), "score_pass")
    selection = required_digest(authorities.get("selection_integration_config_hash"), "selection")
    ...
    authority = {
        ...
        "entries": [
            {"role": role, "schema_id": f"radjax.tome.authority.{role}.v1", "identity": identity}
...
def snapshot_finalized_handoff(prepared: Any, context: Mapping[str, Any]) -> FinalizedV3Handoff:
    ...
    projected = tuple(_closed_record(record, payload) for record, payload in zip(records, payloads, strict=True))
    indexes = tuple(int(record.get("selection_index", index)) for index, record in enumerate(records))
    ...
    return FinalizedV3Handoff(projected, authority, policy, indexes, int(prepared.config.payload_records_per_shard))


def _write_package(root: Path, handoff: FinalizedV3Handoff, transport: str) -> tuple[str, str, int, int]:
    ...
    sequence = record_sequence_digest(records, selection_indexes=handoff.selection_indexes)
    ...
    identity = {**identity_without_root, "semantic_root": semantic_root(identity_without_root)}
    ...
    identity_raw, authority_raw, policy_raw = _raw(identity), _raw(handoff.authority), _raw(handoff.policy)
    caps_raw = _raw({"schema_version": "radjax_tome_capabilities_v1", "required": ["standard_integrity_v3", "streaming_shard_receipts_v3"], "optional": []})
    payload_rows: list[dict[str, Any]] = []
    shard_rows: list[dict[str, Any]] = []
    chunks = [records[i : i + handoff.shard_capacity] for i in range(0, len(records), handoff.shard_capacity)]
    if not chunks:
    ...
    for shard_id, chunk in enumerate(chunks):
        ...
        shard_rows.append({"shard_id": shard_id, "path": path, "sha256": _digest(raw), "size_bytes": len(raw), "first_selection_index": shard_id * handoff.shard_capacity, "record_count": len(chunk)})
        payload_rows.extend({"logical_record_id": logical_record_id(record), "selection_index": shard_id * handoff.shard_capacity + row, "shard_id": shard_id, "row": row} for row, record in enumerate(chunk))
    ...
    layout = {"schema_version": "radjax_tome_payload_layout_v2", "semantic_identity_ref": _ref(identity_path, identity_raw, "radjax_tome_semantic_identity_v3"), "payload_index_ref": _ref(payload_index_path, payload_index_raw, "radjax_tome_payload_index_v3", record_count=len(records)), "shard_index_ref": _ref(shard_index_path, shard_index_raw, "radjax_tome_payload_shard_index_v2", record_count=len(shard_rows)), "record_count": len(records), "shard_capacity": handoff.shard_capacity}
    layout_raw = _raw(layout)
    members: dict[str, tuple[bytes, str]] = {identity_path: (identity_raw, "semantic_identity"), authority_path: (authority_raw, "semantic_authority"), policy_path: (policy_raw, "behavioral_policy"), caps_path: (caps_raw, "capabilities"), layout_path: (layout_raw, "payload_layout"), payload_index_path: (payload_index_raw, "payload_index"), shard_index_path: (shard_index_raw, "payload_shard_index")}
    for row in shard_rows:
        members[row["path"]] = ((root / row["path"]).read_bytes(), "payload_shard")
    inventory_rows = [{"path": path, "sha256": _digest(raw), "size_bytes": len(raw), "member_role": role, "classification": "integrity_or_provenance", "required_for_standard_validation": True} for path, (raw, role) in members.items()]
    ...
    header = {"schema_version": "tome_content_manifest_header_v4", "contract_version": V3_SCHEMA, "profile_id": SEMANTIC_PROFILE_ID, "capabilities_ref": _ref(caps_path, caps_raw, "radjax_tome_capabilities_v1"), "semantic_identity_ref": _ref(identity_path, identity_raw, "radjax_tome_semantic_identity_v3"), "layout_ref": _ref(layout_path, layout_raw, "radjax_tome_payload_layout_v2"), "inventory_ref": _ref(inventory_path, inventory_raw, "tome_content_manifest_inventory_v4"), "entry_count": len(inventory_rows)}
    header_raw = _raw(header)
    cover = {"schema_version": V3_COVER_SCHEMA, "contract_version": V3_SCHEMA, "package": {"profile_id": SEMANTIC_PROFILE_ID, "transport": transport}, "capabilities_ref": _ref(caps_path, caps_raw, "radjax_tome_capabilities_v1"), "semantic_identity_ref": _ref(identity_path, identity_raw, "radjax_tome_semantic_identity_v3"), "semantic_authority_ref": _ref(authority_path, authority_raw, "radjax_tome_semantic_authority_v1"), "behavioral_policy_ref": _ref(policy_path, policy_raw, "radjax_tome_behavioral_policy_v1"), "manifest_header_ref": _ref(header_path, header_raw, "tome_content_manifest_header_v4"), "record_count": len(records), "shard_count": len(shard_rows)}
    ...
    for path, (raw, _) in members.items():
        if not (root / path).exists():
            _fsync_bytes(root / path, raw)
    return identity["semantic_root"], authority_identity, policy_identity, len(shard_rows)


def publish_v3_from_handoff(handoff: FinalizedV3Handoff, output_base: Path) -> V3Publication:
    ...
    staging = Path(tempfile.mkdtemp(prefix=f".{output_base.name}.v3-", dir=output_base.parent))
    try:
        ...
        with tempfile.NamedTemporaryFile(prefix=f".{output_base.name}.v3-", suffix=".tgz", dir=output_base.parent, delete=False) as handle:
        ...
        try:
            ...
            with tarfile.open(archive_tmp, "w:gz") as tar:
                for path in sorted(archive_root.rglob("*")):
                    if path.is_file():
                        tar.add(path, arcname=path.relative_to(archive_root).as_posix(), recursive=False)
            os.replace(archive_tmp, archive)
        finally:
            archive_tmp.unlink(missing_ok=True)
        validate_tome_artifact_v3(archive)
        return V3Publication(directory, archive, semantic, authority, policy, len(handoff.records), shards)
    finally:
        shutil.rmtree(staging, ignore_errors=True)


__all__ = ["FinalizedV3Handoff", "V3Publication", "snapshot_finalized_handoff", "publish_v3_from_handoff"]
