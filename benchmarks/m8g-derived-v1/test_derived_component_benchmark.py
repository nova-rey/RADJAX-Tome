...
def test_derived_manifest_and_samples():
    ...
    assert len({(r["coordinate"]["example_id"], r["coordinate"]["position"]) for r in manifest["records"]}) == 169
    raw = json.loads((root / "raw_nine_sample_report.json").read_text())
    assert len(raw["results"]) == 9
    assert {x["mode"] for x in raw["results"]} == {"legacy_padded_monolithic", "compact_k_monolithic", "compact_k_immutable_body"}
