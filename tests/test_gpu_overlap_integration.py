...
def test_native_c6_compact_uses_buffer_pipeline():
    source = (
        Path(__file__).parents[1] / "src/radjax_tome/builder/delivery/assembly.py"
    ).read_text()
...
def test_pipeline_uses_public_buffer_contract_boundary():
    source = (
        ...
        / "src/radjax_tome/builder/delivery/simple_compact_body.py"
    ).read_text()
    ...
    assert (
        ...
        not in source.split("def write_compact_body_store_pipelined_from_compact", 1)[1]
    )
