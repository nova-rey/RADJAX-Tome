...
def test_compact_payload_to_numpy_keeps_mixed_corridor_source_and_score_fields() -> (
    ...
    payload = {
        ...
        "corridor_entropy": _ArrayTensor(np.ones((2, 3), dtype=np.float32)),
        "corridor_top1_margin": _ArrayTensor(np.ones((2, 3), dtype=np.float32)),
        "corridor_top8_mass": _ArrayTensor(np.ones((2, 3), dtype=np.float32)),
        "corridor_top32_mass": _ArrayTensor(np.ones((2, 3), dtype=np.float32)),
        "corridor_tail_mass": _ArrayTensor(np.zeros((2, 3), dtype=np.float32)),
