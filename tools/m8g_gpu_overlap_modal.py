...
import hashlib, json, os, pathlib, platform, shutil, subprocess, time, uuid
import modal
...
def run_suite():
    ...
    env = {"gpu": subprocess.run(["nvidia-smi", "--query-gpu=name,uuid,driver_version,memory.total", "--format=csv,noheader"], capture_output=True, text=True).stdout.strip(), "torch": torch.__version__, "cuda": torch.version.cuda, "python": platform.python_version(), "cpu_count": os.cpu_count(), "platform": platform.platform()}
    ...
    shaped_iters = max(1, int((2600.0 / one_ms) * 0.95))
    calibration = {"target_ms_per_batch": 2600.0, "calibration_iterations": calibration_iters, "calibration_loop_ms": loop_ms, "one_iteration_ms": one_ms, "iterations": shaped_iters}
    ...
    def gpu_batch(start, cadence, iterations):
        ...
        if cadence == "production_shaped":
            ...
            for _ in range(iterations): work = work @ work.t() * 0.0001
    ...
    def root_digest(res):
        return hashlib.sha256(json.dumps(res["body_digests"], separators=(",", ":")).encode()).hexdigest()

    def serial(cadence, rid):
        root = out / rid; root.mkdir(); payloads = []; t0 = time.perf_counter()
        ...
        return {"variant": "serial_compact", "producer_seconds": producer, "representation_seconds": time.perf_counter() - t, "total_seconds": total, "writer": res, "body_digest_root": root_digest(res), "metadata_sha256": res["metadata_sha256"], "root": str(root)}

    def pipeline(cadence, rid):
        root = out / rid
        stream = descriptor_stream_from_cuda(ks, vocab_size=vocab, batch_size=8, cadence=cadence, shaped_iterations=shaped_iters, torch_module=torch)
        ...
        return {"variant": "pipelined_compact", "total_seconds": total, "writer": res, "body_digest_root": root_digest(res), "metadata_sha256": res["metadata_sha256"], "root": str(root), **{k: v for k, v in res.items() if k not in {"body_digests", "metadata_path", "metadata_sha256"}}}

    def lower(cadence, rid):
        t = time.perf_counter()
        for start in range(0, len(ks), 8): gpu_batch(start, cadence, shaped_iters)
        torch.cuda.synchronize()
        return {"variant": "producer_lower_bound", "total_seconds": time.perf_counter() - t, "root": None}
    ...
    for cadence in ("maximum_pressure", "production_shaped"):
        for rep in range(3):
            for variant in ("producer_lower_bound", "serial_compact", "pipelined_compact"):
                ...
                rec.update({"cadence": cadence, "rep": rep, "run_id": rid, "batch_count": (len(ks) + 7) // 8, "calibration": calibration})
                if rec.get("root"):
    ...
    for cadence in ("maximum_pressure", "production_shaped"):
        for rep in range(3):
            ...
            for x in group: x["logical_equivalence"] = len(roots) == 1 and len(metas) == 1
    payload = {"environment": env, "calibration": calibration, "k_count": len(ks), "full_width_count": sum(k == vocab for k in ks), "results": results}
    pathlib.Path("/mnt/overlap/results.json").write_text(json.dumps(payload, sort_keys=True, indent=2))
    return json.dumps({"environment": env, "calibration": calibration, "result_count": len(results), "results_path": "/mnt/overlap/results.json"})
